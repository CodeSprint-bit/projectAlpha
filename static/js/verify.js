...
document.addEventListener('DOMContentLoaded', function() {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (!docType) { showError('Please select a document type.'); return; }
    if (!name) { showError('Please enter your name.'); return; }
    if (!docNum) { showError('Please enter the document number.'); return; }
    if (!file || file.size === 0) { showError('Please choose a document image.'); return; }
  ...
  function displayResults(data) {
    ...
    document.getElementById('riskLevel').className = 'badge ' + getRiskClass(riskLevel);
    document.getElementById('riskScore').textContent = riskScore;

    // Document
    const doc = data.document || {};
    document.getElementById('enteredNumber').textContent = doc.entered_number || '--';
    document.getElementById('detectedNumber').textContent = doc.detected_number || 'Not detected';
    // number_match
